...
class Gui():
    def __init__(self):
        ...
        self.keyboard_gui = {
            ...
            "Z": None, "X": None, "C": None, "V": None, "B": None, "N": None, "M": None,
        }
        ...
        self.canvas = tk.Canvas(self.window, width=500, height=700, background="white")
        self.canvas.pack()

        self.draw_word_box(100, 50)
    ...
    def on_enter_pressed(self, event):
        print("ENTER")
        if len(self.current_trial) == 5 and self.current_trial.lower() in words.words():
            for i in range(5):
                if self.current_trial[i] == self.word[i]:
                    self.fill_the_boxes(i, self.current_trial[i], "GREEN")
                elif self.current_trial[i] in self.word:
    ...
    def get_user_input(self):
        ...
        if user_input.lower() in words.words() and len(user_input) == 5:
            self.word = user_input.upper()
            self.refresh_canvas()
        else:
            self.get_user_input()
    ...
    def draw_keyboard_box(self, start_x, start_y):
        ...
        # draw the first row
        draw_row(self.letters[0], start_x, start_y)
        # draw the second row
